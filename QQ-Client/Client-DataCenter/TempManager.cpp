...
{
	connect(EventBus::instance(), &EventBus::notice_addFriend, this, [=](const QJsonObject& obj, const QPixmap& pixmap)
		{
			auto user_id = obj["user_id"].toString();
			addFriendRequest(user_id, obj, pixmap);
			emit friendAddRequest(user_id);
		});
	connect(EventBus::instance(), &EventBus::notice_addGroup, this, [=](const QJsonObject& obj, const QPixmap& pixmap)
		{
			auto group_id = obj["group_id"].toString();
			addGroupRequest(group_id, obj, pixmap);
			emit groupAddRequest(group_id);
		});
	connect(EventBus::instance(), &EventBus::notice_friendDeleted, this, [=](const QJsonObject& obj, const QPixmap& pixmap)
		{
			auto user_id = obj["user_id"].toString();
			addFriendReply(user_id, obj, pixmap);
			emit friendDeleted(user_id);
		});
	connect(EventBus::instance(), &EventBus::notice_rejectAddFriend, this, [=](const QJsonObject& obj, const QPixmap& pixmap)
		{
			auto user_id = obj["user_id"].toString();
			addFriendReply(user_id, obj, pixmap);
			emit friendAddReply(user_id);
		});
	connect(EventBus::instance(), &EventBus::notice_groupInvite, this, [=](const QJsonObject& obj, const QPixmap& pixmap)
		{
			auto group_id = obj["group_id"].toString();
			addGroupRequest(group_id, obj, pixmap);
			emit GroupInvite(group_id);
		});
	connect(EventBus::instance(), &EventBus::notice_groupInviteSuccess, this, [=](const QJsonObject& obj, const QPixmap& pixmap)
		{
			auto group_id = obj["group_id"].toString();
			addGroupReply(group_id, obj, pixmap);
			emit GroupInviteSuccess(group_id);
		});
	connect(EventBus::instance(), &EventBus::notice_groupMemberExitGroup, this, [=](const QJsonObject& obj, const QPixmap& pixmap)
		{
			auto group_id = obj["group_id"].toString();
			addGroupReply(group_id, obj, pixmap);
			emit groupMemberExitGroup(group_id);
		});
	connect(EventBus::instance(), &EventBus::notice_rejectAddGroup, this, [=](const QJsonObject& obj, const QPixmap& pixmap)
		{
			auto group_id = obj["group_id"].toString();
			addGroupReply(group_id, obj, pixmap);
			emit rejectAddGroup(group_id);
		});
	connect(EventBus::instance(), &EventBus::notice_groupAddFailed, this, [=](const QJsonObject& obj, const QPixmap& pixmap)
		{
			auto group_id = obj["group_id"].toString();
			addGroupReply(group_id, obj, pixmap);
